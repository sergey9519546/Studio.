...
import { Search, Filter, Image, Trash2, X } from 'lucide-react';
  ...
  const filterItems = (query: string, tags: string[]) => {
    ...
    if (tags.length > 0) {
      results = results.filter(item =>
        tags.some(tag =>
          item.tags.includes(tag) || item.moods.includes(tag)
        )
      );
    }
  ...
  return (
    <div className="w-full h-full bg-app">
      <div className="max-w-7xl mx-auto p-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-ink-primary mb-2">Moodboard</h1>
          <p className="text-ink-secondary">Visual inspiration with AI semantic tagging</p>
        </div>
            ...
            <Card className="max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              ...
              {filteredItems.find(i => i.id === selectedItem) && (
                <div>
                  ...
                  />
                  <div>
                    <h3 className="text-lg font-bold text-ink-primary mb-4">Tags & Metadata</h3>
                    <div className="space-y-3">
                      <div>
                        <h4 className="text-sm font-bold text-ink-secondary mb-2">Visual Tags</h4>
                        <div className="flex flex-wrap gap-2">
                          {filteredItems.find(i => i.id === selectedItem)?.tags.map(tag => (
                            <span key={tag} className="px-3 py-1 rounded-[12px] bg-primary-tint text-primary text-xs font-medium">
                              {tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-ink-secondary mb-2">Moods</h4>
                        <div className="flex flex-wrap gap-2">
                          {filteredItems.find(i => i.id === selectedItem)?.moods.map(mood => (
                            <span key={mood} className="px-3 py-1 rounded-[12px] bg-edge-teal/20 text-edge-teal text-xs font-medium">
                              {mood}
                            </span>
                          ))}
                        </div>
