...
export class AIController {
    ...
    async chat(
        ...
        const codeContextMetadata = {
            ...
            files: ragResponse.sources.map((s: { metadata?: RAGSourceMetadata }) =>
...
`;
        ...
        if (typeof response === 'object' && 'toolCalls' in response && response.toolCalls) {
    ...
    async generateEmbeddings(
        ...
        return (this.rag as unknown as { embeddings: { generateEmbedding: (text: string) => Promise<unknown> } }).embeddings.generateEmbedding(body.text);
    }
    ...
    async getStatus(): Promise<Record<string, unknown>> {
        ...
        const vectorStore = stats.vectorStore as { totalDocuments?: number } | undefined;
        return {
            ...stats,
    ...
    @HttpCode(HttpStatus.OK)
    async analyzeProject(@Param('id') _id: string): Promise<ProfitabilityAnalysisResult> {
        return this.aiService.analyzeProjectProfitability(_id);
    }

    @Post('analyze/freelancer/:id')
    @HttpCode(HttpStatus.OK)
    async analyzeFreelancer(@Param('id') _id: string): Promise<PerformanceAnalysisResult> {
    ...
    @Post('search')
    @HttpCode(HttpStatus.OK)
    async search(@Body() body: { query: string; model?: string }): Promise<{ query: string; response: string; provider: string }> {
        if (!body.query) {
            throw new BadRequestException('Query is required');
        }

        const result = await this.geminiService.generateWithGoogleSearch(body.query, body.model);
    ...
    async executeCode(
        ...
        return this.geminiService.executeCode(body.code, body.language, { model: body.model });
    }
    ...
    ): Promise<{ imageUrl?: string; savePath?: string; prompt: string; metadata?: Record<string, unknown> }> {
        if (!body.prompt) {
    ...
    ): Promise<{ query: string; results: string; sources: string[]; confidence: number }> {
        if (!body.query) {
            throw new BadRequestException('Query is required');
        }
    ...
    async multiTool(
    ...
    ): Promise<{ input: string; response: string; toolsUsed: string[]; results: unknown[] }> {
        if (!body.input) {
        ...
        return this.geminiService.createMultiToolInteraction(body.input, body.tools, {
            model: body.model,
            agent: body.agent,
            background: body.background,
        });
    ...
    async batchProcess(
        ...
        successful: number;
        failed: number;
        results: Array<{ prompt: string; success: boolean; result?: string; error?: string; processingTime?: number }>;
    }> {
        if (!body.prompts || body.prompts.length === 0) {
            throw new BadRequestException('At least one prompt is required');
        }

        const results = await this.geminiService.batchGenerateContent(body.prompts, {
    ...
    async customConfig(
        ...
        return this.geminiService.generateWithCustomConfig(body.prompt, body.config ?? {});
    }
    ...
    async analyzeQuality(
        ...
        return this.geminiService.analyzeContentQuality(body.content, body.criteria ?? {});
    }
    ...
    async generateMoodboard(
        ...
        imagery: string[];
        layout: string;
        generatedImages?: Array<{ url: string; description: string; style: string }>;
        metadata: { style: string; projectType: string; targetAudience: string; mood: string[] };
    }> {
        if (!body.theme) {
     ...
     */
    @Get('health')
    async getHealth(): Promise<{ status: string; configured: boolean; provider: string; mode: string }> {
