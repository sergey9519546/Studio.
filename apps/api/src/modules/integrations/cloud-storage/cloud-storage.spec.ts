...
describe('CloudStorageService', () => {
  ...
  it('should throw for unsupported provider in listFiles', async () => {
    await expect(service.listFiles(1, 'unknown-provider', undefined)).rejects.toThrow(
  ...
  it('should throw when user is not connected to provider', async () => {
    ...
    await expect(service.listFiles(1, CloudProviderType.GOOGLE_DRIVE, undefined)).rejects.toThrow(
      new BadRequestException(`User is not connected to ${CloudProviderType.GOOGLE_DRIVE}`)
    );
  ...
  it('should return files from adapter when connected', async () => {
    ...
    mockGoogleDriveAdapter.listFiles.mockResolvedValueOnce({
      files: [
        { id: '1', name: 'file1', mimeType: 'text/plain', modifiedTime: '2025-01-01', size: '123' },
      ],
      nextPageToken: 'next-page',
    });

    const result = await service.listFiles(1, CloudProviderType.GOOGLE_DRIVE, 'folder-1');

    expect(mockGoogleDriveAdapter.isConnected).toHaveBeenCalledWith(1);
    expect(mockGoogleDriveAdapter.listFiles).toHaveBeenCalledWith(1, 'folder-1');
