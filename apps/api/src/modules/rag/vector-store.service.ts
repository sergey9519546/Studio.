...
export class VectorStoreService {
    ...
    if (filter) {
      vectors = vectors.filter((vec) => {
        return Object.entries(filter).every(([key, value]) => {
          ...
          return (
            Object.prototype.hasOwnProperty.call(vec.metadata, key) &&
  ...
  getStats(): { totalDocuments: number; memoryUsage: string } {
    ...
    const memoryBytes = JSON.stringify(
      Array.from(this.vectorStore.values())
    ).length;
  ...
  private async loadVectorsFromDatabase(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Loading ${sources.length} knowledge sources from database...`
      );
