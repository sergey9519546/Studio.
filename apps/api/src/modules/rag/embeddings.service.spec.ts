...
describe("EmbeddingsService", () => {
  ...
  let providerMock: jest.Mocked<EmbeddingsProvider>;

  const makeVector = (len = 4, val = 1): number[] =>
    Array.from({ length: len }, (_, i) => val + i);
  ...
  beforeEach(async () => {
    const mockProvider = {
      ...
      healthCheck: jest.fn().mockResolvedValue({ status: "ok", model: "test" }),
    };
    ...
    providerMock = module.get(EMBEDDINGS_PROVIDER);
  });
  ...
  it("should use cached embedding within TTL", async () => {
    ...
    providerMock.generateEmbedding.mockResolvedValueOnce(vec);

    const first = await service.generateEmbedding(text);
    expect(providerMock.generateEmbedding).toHaveBeenCalledTimes(1);
  ...
  it("should evict oldest when cache is full", async () => {
    ...
    providerMock.generateBatchEmbeddings.mockResolvedValueOnce(vectors);
    ...
    providerMock.generateBatchEmbeddings.mockResolvedValueOnce([newVec]);
    ...
    providerMock.generateEmbedding.mockResolvedValueOnce(newVec);
  ...
  it("should mix cached and new embeddings in batch", async () => {
    ...
    providerMock.generateBatchEmbeddings.mockResolvedValueOnce([
      makeVector(4, 1),
      makeVector(4, 2),
      makeVector(4, 3),
    ]);
    ...
    expect(providerMock.generateBatchEmbeddings).toHaveBeenCalledTimes(1);
  ...
  it("should compute similarities and return topK sorted", async () => {
    ...
    providerMock.generateEmbedding.mockResolvedValueOnce(qVec);
    providerMock.generateBatchEmbeddings.mockResolvedValueOnce(dVecs);
  ...
  it("should propagate error when query embedding fails", async () => {
    providerMock.generateEmbedding.mockRejectedValueOnce(new Error("boom"));
  ...
  it("should clear expired cache entries", async () => {
    ...
    providerMock.generateBatchEmbeddings.mockResolvedValueOnce(vecs);
    ...
    providerMock.generateBatchEmbeddings.mockResolvedValueOnce(vecs);
    const res = await service.generateBatch(texts);
    expect(providerMock.generateBatchEmbeddings).toHaveBeenCalledTimes(2);
