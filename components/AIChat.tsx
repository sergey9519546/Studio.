      ...
      {parts.map((part, index) => {
        if (part.startsWith("```")) {
          ...
          return (
            <div
              key={index}
              className="bg-gray-900 text-gray-100 p-3 rounded-md font-mono text-xs my-2 overflow-x-auto custom-scrollbar"
            >
              <pre>{content}</pre>
            </div>
          );
...
const AIChat: React.FC<AIChatProps> = ({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.toolCalls) {
        ...
        if (!finalResponse.ok)
          throw new Error("Failed to fetch final response");

        const finalData = await finalResponse.json();
        setMessages((prev) => [
          ...prev,
          { role: "model", parts: [{ text: finalData.response }] },
        ]);
      } else {
        setMessages((prev) => [
          ...prev,
      ...
      setMessages((prev) => [
        ...prev,
        { role: "model", parts: [{ text: `Error: ${errorMessage}` }] },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div
      className={`flex flex-col h-full bg-white ${agentMode ? "" : "rounded-2xl border border-border-subtle shadow-sm"}`}
    >
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold text-ink-primary">
              {customTitle || "Studio Intelligence"}
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-app/30">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-sm text-ink-secondary max-w-xs mb-8 leading-relaxed">
              I can help analyze project data, draft communications, or find the
              perfect freelancer for your next campaign.
            </p>
        ...
        {messages.map((msg, i) => (
          <div
            key={i}
            className={`flex gap-4 ${msg.role === "user" ? "flex-row-reverse" : ""} animate-enter`}
          >
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${msg.role === "user" ? "bg-ink-primary text-white" : "bg-white border border-border-subtle text-primary"}`}
            >
              ...
              {msg.files && msg.files.length > 0 && (
                <div className="flex flex-wrap gap-2 justify-end">
                  {msg.files.map((f: string, idx: number) => (
                    <div
                      key={idx}
                      className="text-[10px] bg-subtle text-ink-secondary px-2 py-1 rounded-md flex items-center gap-1 border border-border-subtle font-medium"
                    >
                      <FileText size={10} /> {f}
                    </div>
                  ))}
        ...
        {selectedFiles.length > 0 && (
          <div className="flex gap-2 mb-3 overflow-x-auto pb-1 custom-scrollbar">
            {selectedFiles.map((file, i) => (
              <div
                key={i}
                className="flex items-center gap-2 bg-primary-tint/20 text-primary px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap border border-primary/20"
              >
                <FileText size={12} />
