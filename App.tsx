...
const DashboardWrapper: React.FC<DashboardWrapperProps> = (props) => {
  ...
  return (
    <Dashboard
      {...props}
      onCallAction={(action, params) =>
        props.onAgentAction(action, params, navigate)
      }
    />
  );
};
...
const AppContent: React.FC = () => {
  ...
  const [freelancers, setFreelancers] = useState<Freelancer[]>(() =>
    loadFromStorage("freelancers", [])
  );
  const [projects, setProjects] = useState<Project[]>(() =>
    loadFromStorage("projects", [])
  );
  const [assignments, setAssignments] = useState<Assignment[]>(() =>
    loadFromStorage("assignments", [])
  );
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      setFreelancers(
        freelancersData.filter(
          (item): item is Freelancer => item !== undefined && item !== null
        )
      );
      setProjects(
        projectsData.filter(
          (item): item is Project => item !== undefined && item !== null
        )
      );
      setAssignments(
        assignmentsData.filter(
          (item): item is Assignment => item !== undefined && item !== null
        )
      );
      // setScripts(s.data || []);
    } catch (e) {
  ...
  const handleImport = async (
    ...
    try {
      if (type === "freelancer") {
        ...
        handleLogAction(
          "Import Completed",
          `Imported/Updated ${(res.data?.created || 0) + (res.data?.updated || 0)} freelancers.`
        );
        toast.success(
          `Successfully imported ${(res.data?.created || 0) + (res.data?.updated || 0)} freelancers`
        );
      } else {
        const res = await api.projects.importBatch(data as Project[]);
        handleLogAction(
          "Import Completed",
          `Imported/Updated ${(res.data?.created || 0) + (res.data?.updated || 0)} projects.`
        );
        toast.success(
          `Successfully imported ${(res.data?.created || 0) + (res.data?.updated || 0)} projects`
        );
      }
      await fetchData();
    } catch (e) {
  ...
  const handleFreelancerUpdate = async (updatedFreelancer: Freelancer) => {
    ...
    if (res.data) {
      ...
      handleLogAction(
        "Freelancer Updated",
        `Updated profile for ${res.data!.name}`
      );
  ...
  const handleProjectDelete = async (id: string) => {
    ...
    handleLogAction(
      "Project Deleted",
      "Deleted project and associated assignments"
    );
  ...
  const handleBulkProjectDelete = async (ids: string[]) => {
    if (
      ...
      try {
        ...
        setAssignments((prev) =>
          prev.filter((a) => !ids.includes(a.projectId))
        );
  ...
  const handleAssignmentConfirm = async (assignment: Assignment) => {
    const newAssignment = { ...assignment };
    const existing = assignments.find((a) => a.id === newAssignment.id);
    let res: Awaited<
      ReturnType<typeof api.assignments.update | typeof api.assignments.create>
    >;
    ...
    try {
      if (existing) {
        ...
        if (res.data) {
          setAssignments((prev) =>
            prev.map((a) => (a.id === newAssignment.id ? res.data! : a))
          );
        }
        ...
        if (res.data) {
          ...
          const freelancerName =
            freelancers.find((f) => f.id === res.data!.freelancerId)?.name ||
            "Unknown";
          const projectName =
            projects.find((p) => p.id === res.data!.projectId)?.name ||
            "Unknown";
          handleLogAction(
            "Assignment Confirmed",
            `${freelancerName} assigned to ${projectName}`
          );
      ...
      if (e instanceof Error && e.message === "CONFLICT_DETECTED") {
        toast.error(
          "Server Conflict: Assignment overlaps with existing booking."
        );
      } else {
  ...
  const handleFreelancerCreate = async (overrides?: Partial<Freelancer>) => {
    ...
    handleLogAction(
      "Talent Added",
      `Added new freelancer: ${newFreelancer.name}`
    );
  ...
  const handleScriptCreate = async (script: Script) => {
    ...
    // });
    const projName =
      projects.find((p) => p.id === script.projectId)?.name ||
      "Unknown Project";
    ...
    switch (action) {
      ...
      case "assign_freelancer": {
        const assignParams: Assignment = {
          ...
          endDate:
            params.endDate || new Date(Date.now() + 86400000 * 7).toISOString(),
          allocation: 100,
          status: "Confirmed",
        };
          ...
          <Route
            ...
            element={
              <ProjectList
                ...
                onBulkDelete={handleBulkProjectDelete}
                isLoading={isLoading}
              />
            }
          />
          <Route
            ...
            element={
              <ProjectDetail
                ...
                freelancers={freelancers}
                assignments={assignments}
                onCreate={handleAssignmentConfirm}
          ...
          <Route
            ...
            element={
              <FreelancerList
                freelancers={freelancers}
                assignments={assignments}
                onCreate={() => handleFreelancerCreate()}
              />
            }
          />
          <Route
            ...
            element={
              <FreelancerDetail
                ...
                projects={projects}
                assignments={assignments}
                onUpdate={handleFreelancerUpdate}
                onDelete={handleFreelancerDelete}
              />
            }
          />
          ...
          <Route
            ...
            element={
              <CreateStudio
                ...
                freelancers={freelancers}
                assignments={assignments}
                onSaveScript={handleScriptCreate}
              />
            }
          />
